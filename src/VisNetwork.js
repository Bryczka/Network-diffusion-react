      ...
      <div style={{ display: "flex", height: "100%", flexDirection: "row" }}>
        <div style={{ flex: 1, backgroundColor: "lightgray", overflowY: "scroll" }}>
          ...
          <Card style={{ margin: 10, padding: 20 }}>
            ...
            <p>Średni stopień wierzchołka: {this.state.networkMeanDegree}</p>
            <p>Średni współczynnik klasteryzacji: {this.state.networkMeanClusterinRate}</p>
          </Card>
        </div>
  ...
  handleNetworkRequest = (networkParameters) => {
    fetch("https://localhost:44316/network/" + networkParameters.networkType + "/" + networkParameters.nodesCount)
      ...
      .then((responseData) => {
        ...
        // this.setState({ networkMeanDegree: responseData.networkStatistics.meanDegree });
        // this.setState({ networkMeanClusterinRate: responseData.networkStatistics.meanClusteringRate });
      });
  };
}
